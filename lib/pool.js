...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
		    ...
		    // Can use this area for parsing ID's of Workers - Wallet.Diff#WorkerID
	    ...
	    //LETS SEE
	    if(config.poolServer.Minions.enabled) {
		var workerIDCharPos = login.indexOf(config.poolServer.Minions.addressSeparator);    
	    }
